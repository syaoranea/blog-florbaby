...
import { CategoriaModel, ListJsModel } from '../../tables/listjs/listjs.model';
import { listSortEvent , NgbdListSortableHeader} from '../../tables/listjs/listjs-sortable.directive';
import { APIService, CreateCategoriaInput, CreateTagsInput, DeleteCategoriaInput,  DeleteTagsInput,  UpdatePostagemInput, UpdateTagsInput } from 'src/app/API.service';
  ...
  saveListJs() {
    ...
    if (this.listJsForm.valid) {
      ...
      if (this.listJsForm.get('ids')?.value) {
        ...
        this.api.CreateTags(postDetails as CreateTagsInput).then(res=>{
          console.log(res)
        }).catch(error=>{
          console.log(error)
        });
